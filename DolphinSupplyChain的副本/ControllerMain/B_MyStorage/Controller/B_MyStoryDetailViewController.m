//
//  B_MyStoryDetailViewController.m
//  DolphinSupplyChain
//
...
@interface B_MyStoryDetailViewController ()<UIScrollViewDelegate, B_MyStoryScroviewDelegate,  CommodityFootDelegate, ImageAndTextDelegate>


@property (nonatomic, copy) NSString *sSku;

@property (nonatomic, strong) B_MyStoryScroview *scrDetails;
...
@implementation B_MyStoryDetailViewController
...
- (void)initAddView
{
    ...
    [self addNavigationType:YKS_Left_Title_RightOne_RightTwo NavigationTitle:@"微仓商品详情"];
...
- (void)prentShopingInforViewController{
    ...
    DetailShopInforViewController * VC = [[DetailShopInforViewController alloc]init];
    VC.sku_in = self.sSku;
    [self.navigationController pushViewController:VC animated:YES];
}
...
-(void)scrollViewDidEndDragging:(UIScrollView *)scrollView willDecelerate:(BOOL)decelerate
{
    if (scrollView.tag == 100)
    {
        ...
        {
            [UIView animateWithDuration:0.4 animations:^{
                self.scrDetails.frame = CGRectMake(0, -(kDisHeight - kDisNavgation - 43 - kDisNavgation), kDisWidth, kDisHeight - kDisNavgation - 43);
                self.vImageAndText.frame = CGRectMake(0, kDisNavgation, kDisWidth, 40);
                self.scrImageAndText.frame = CGRectMake(0, kDisNavgation + 40, kDisWidth, kDisHeight - kDisNavgation - 40 - 43);
            } completion:^(BOOL finished) {
                self.bUpAnimation = NO;
...
#pragma mark - 懒加载
- (B_MyStoryScroview *)scrDetails
{
    if (!_scrDetails) {
        _scrDetails = [[B_MyStoryScroview alloc] initWithDetailsScroll];
        _scrDetails.frame = CGRectMake(0, kDisNavgation, kDisWidth, kDisHeight - kDisNavgation - 43);
...
- (TextScrollView *)scrText
{
    if (!_scrText) {
        ...
        _scrText.frame = CGRectMake(kDisWidth, 0, kDisWidth, kDisHeight - kDisNavgation - 43 - 40-KBottom);
