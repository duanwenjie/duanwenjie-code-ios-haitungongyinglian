...
static NSString * const kOrderSubmitURL1 = @"/SalesOrder/editSalesOrder";

static CGFloat  const tableHeight1 = 75;

static CGFloat  const textFieldHeight1 = 40;

static CGFloat  const kSpecLeft1 = 10;

NSString *const YKSAllSellOrderCount1 = @"YKSAllSellOrderCount";

NSString *const YKSShortageSellOrderCount1 = @"YKSShortageSellOrderCount";

NSString *const YKSDeliveGoodsSellOrderCount1 = @"YKSDeliveGoodsSellOrderCount";

@interface ChangeOrderViewController ()<UITextFieldDelegate,UITextViewDelegate,UIActionSheetDelegate,FoundOrEditDelegate,UITableViewDataSource,UITableViewDelegate,GetGoodsViewControllerDelegate,UIAlertViewDelegate,UIScrollViewDelegate,GoodsExtentionViewControllerDelegate>
{
...
-(void)setUI{
    ...
    backView = [[BackScrollView alloc] initWithFrame:CGRectMake(0, kDisNavgation, kDisWidth, kDisHeight-KBottom)];
    backView.contentSize = CGSizeMake(kDisWidth, kDisHeight-KBottom);
    backView.showsVerticalScrollIndicator = NO;
    backView.delegate = self;
    backView.backgroundColor = color(239, 239, 244, 1.0);
    [self.view addSubview:backView];
    
    UIView *orderView = [[UIView alloc] initWithFrame:CGRectMake(0, 10, kDisWidth, textFieldHeight1)];
    orderView.backgroundColor = [UIColor whiteColor];
    [backView addSubview:orderView];
    
    order_id = [[UITextField alloc] initWithFrame:CGRectMake(kSpecLeft1, 0, kDisWidth-15.0, textFieldHeight1)];
    ...
    order_id.userInteractionEnabled = NO;
    [orderView addSubview:order_id];
    
    UILabel *labZ = [[UILabel alloc] initWithFrame:CGRectMake(kSpecLeft1, orderView.bottom + 3, kDisWidth, 20)];
    ...
    quicklyView.frame = CGRectMake(0, platView.bottom+5, kDisWidth, textFieldHeight1);
    ...
    //    quickBtn.center = CGPointMake(25, view1.height/2);
    ...
    submitBtn=[[UIButton alloc] initWithFrame:CGRectMake(0, 5-KBottom, kDisWidth, 35.0)];
...
//    submitBtn.layer.cornerRadius=3.0;
    ...
    [self addNavigationType:YKSDefaults NavigationTitle:@"修改订单"];
...
#pragma mark --订单详情数据
-(void)setChangeViewControllerData:(BCOrderSellDetail *)sellModel{

    [self setUI];
    order_id.text = sellModel.sales_order_sn;
    self.sales_order_id = sellModel.sales_order_id;
    name.text = sellModel.consignee;
    identity.text = sellModel.id_card_number;
    phoneNum.text = sellModel.mobile;
    areaText.text = [NSString stringWithFormat:@"%@   %@   %@",sellModel.province,sellModel.city,sellModel.district];
    address.textColor = [UIColor colorWithHexString:@"#666666"];
    address.text = sellModel.address;
    provinceStr = sellModel.province;
    cityStr = sellModel.city;
    districtStr = sellModel.district;
    if ([sellModel.consumer_note isEqualToString:@""]) {
        labb_lab.text = @"(选填)控制在20个字以内";
    }else{
        labb_lab.textColor = [UIColor colorWithHexString:@"#666666"];
        labb_lab.text = sellModel.consumer_note;
    }
    NSMutableArray * arrM = [[NSMutableArray alloc]init];
    for (BCorderSellDetailBuyModel * model in sellModel.arrGoods) {
        GoodsAddModel * addModel = [[GoodsAddModel alloc]init];
        
        NSDictionary * dict = [[NSDictionary alloc]init];
        dict = @{
                 @"goods_name":model.goods_name,
                 @"goods_sn":model.sku,
                 @"goods_amount":model.quantity,
                 @"goods_imgthumb":model.img_thumb,
                 @"min_sale_quantity":model.min_sale_quantity,
                 @"is_modulo":model.is_modulo,
                 @"img_original":model.img_original
                 };
        [addModel setValues:dict];
...
- (void)checkInOrder{
    ...
    }else
    {
        ...
        if (is_check) {
            ...
            check = @"no";
        NSDictionary * dic = @{ @"sales_order_id":self.sales_order_id,
                                @"consignee":name.text,
        ...
        [AFHTTPClient POST:kOrderSubmitURL1 params:dic successInfo:^(ResponseModel *response) {
...
-(void)makeTableViewHeightChange{
    [table setFrame:CGRectMake(10.0, lab33.bottom+5.0, kDisWidth-20.0, tableHeight1*[productList count])];
...
#pragma mark --GoodsExtentionViewDelegate
